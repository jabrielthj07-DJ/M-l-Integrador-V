...
const idVenta = parseInt(params.get("id"));
...
async function cargarFactura() {
    ...
    try {
        ...
        document.getElementById("idVenta").textContent =
            `FAC-${String(venta.idVenta).padStart(5, "0")}`;

        document.getElementById("fecha").textContent =
            new Date(venta.fechaVenta).toLocaleDateString("es-NI");
        ...
        (venta.detalles || []).forEach(detalle => {
            ...
            tbody.innerHTML += `
                <tr>
                    ...
                    <td>C$ ${Number(detalle.precioUnitario).toFixed(2)}</td>
